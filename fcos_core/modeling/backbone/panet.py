...
class PANet(nn.Module):
    """
    Module that adds PANet on top of a list of feature maps.
    ...
    def forward(self, x):
        ...
        fpn = []
        fpn.append(getattr(self, self.layer_blocks[-1])(last_inner))
        for feature, inner_block, layer_block in zip(
            ...
            fpn.insert(0, getattr(self, layer_block)(last_inner))

        results = [fpn[0]]
        last_n = fpn[0]  # N3 is P3, without any processing
        for feature, down_block, second_block in zip(fpn[1:], self.panet_down_blocks, self.panet_second_blocks):
            nx_tmp = getattr(self, down_block)(last_n)
            nx_tmp = nx_tmp + feature
            last_n = getattr(self, second_block)(nx_tmp)
            results.append(last_n)
