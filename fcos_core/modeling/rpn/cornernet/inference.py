...
class CornerNetPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, out, image_sizes, K, num_dets, stride):
        """
        Arguments:
        ...
        bboxes = _gather_feat(bboxes, inds) * stride
    ...
    def forward(self, outs, image_sizes):
        """
        Arguments:
            outs: list[Tensor]
        ...
        for level, out in enumerate(outs):
            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    out, image_sizes, self.K[level], self.num_dets[level], self.strides[level]
                )
            )
...
def make_cornernet_postprocessor(config):
    ...
    strides = config.MODEL.FCOS.FPN_STRIDES
